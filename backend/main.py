...
# Create the uploads folder if missing (parents=True allows nested folders)
...
async def update_invoice(invoice_id: int, invoice_data: dict):
    ...
    If there's an existing file on disk, we keep its extension. 
    If there's no old file, we do NOT force ".pdf" for new naming.
    Payment is stored in the 'payment' column.
    """
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        ...
        # 2) Prepare new merchant/order_number
        merchant = (invoice_data.get("merchant") or "").strip()
        order_number = (invoice_data.get("order_number") or "").strip()

        # 3) Determine old extension if a file existed
        old_ext = ""
        if old_filename:
            _, old_ext = os.path.splitext(old_filename)  # e.g. ".jpg" or ".pdf"

        # 4) If merchant + order_number exist, rename using old extension (if any)
        if merchant and order_number:
            new_filename = f"{merchant} - Order# {order_number}{old_ext}"
        else:
            # If missing merchant/order_number, keep old filename
            new_filename = old_filename

        # 5) If old file actually exists and the name changed, rename on disk
        ...
        # 6) Convert tags and line_items to JSON
        ...
        cur.execute(
            ...
            (
                ...
                invoice_data.get("payment", ""),  # store 'payment'
