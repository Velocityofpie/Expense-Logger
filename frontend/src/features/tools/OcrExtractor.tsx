...
// Define the API URL with a default fallback
  ...
  const fetchLanguages = async (): Promise<void> => {
    try {
      ...
      if (response.ok) {
        ...
        const languagesWithNames = data.languages.map((lang: string | Language) => {
          // If it's an object with code/name already
          if (typeof lang === 'object' && 'code' in lang) {
            return {
              ...lang,
          ...
          return {
            code: lang as string,
            name: LANGUAGE_NAMES[lang as string] || lang
          };
        });
